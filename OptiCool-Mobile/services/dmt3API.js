...
const adjustACTempAPI = async (tempValue = 24) => {
  ...
  Alert.alert(
    "Setting AC Temp Error",
    "Error occured when setting AC temperature to low / high"
  );
...
const getPowerConsumptionAPI = async () => {
  ...
  try {
    const { data } = await axios.get(
      `${dmtUrl}/power_consumption_data?start_date=${earliestDate}&end_date=${today}`
    );
    console.log(data);
    return data;
  } catch (error) {
...
const getInsideHumidityAPI = async () => {
  const earliestDate = "2024-08-01";
  const today = new Date().toISOString().split("T")[0];

  try {
    const { data } = await axios.get(
      `${dmtUrl}/inside_humidity_data?start_date=${earliestDate}&end_date=${today}`
    );
    return data;
  } catch (error) {
    console.error("Error fetching inside humidity data:", error);
    return null;
  }
};

const getOutsideHumidityAPI = async () => {
  const earliestDate = "2024-08-01";
  const today = new Date().toISOString().split("T")[0];

  try {
    const { data } = await axios.get(
      `${dmtUrl}/outside_humidity_data?start_date=${earliestDate}&end_date=${today}`
    );
    return data;
  } catch (error) {
    console.error("Error fetching outside humidity data:", error);
    return null;
  }
};

const getInsideTemperatureAPI = async () => {
  const earliestDate = "2024-08-01";
  const today = new Date().toISOString().split("T")[0];

  try {
    const { data } = await axios.get(
      `${dmtUrl}/inside_temperature_data?start_date=${earliestDate}&end_date=${today}`
    );
    return data;
  } catch (error) {
    console.error("Error fetching inside temperature data:", error);
    return null;
  }
};

const getOutsideTemperatureAPI = async () => {
  const earliestDate = "2024-08-01";
  const today = new Date().toISOString().split("T")[0];

  try {
    const { data } = await axios.get(
      `${dmtUrl}/outside_temperature_data?start_date=${earliestDate}&end_date=${today}`
    );
    return data;
  } catch (error) {
    console.error("Error fetching outside temperature data:", error);
    return null;
  }
};
...
export default {
  ...
  getOutsideHumidityAPI,
  getInsideTemperatureAPI,     
  getOutsideTemperatureAPI,
  getWeeklyUsageAPI,
  getMonthlyUsageAPI,
};
