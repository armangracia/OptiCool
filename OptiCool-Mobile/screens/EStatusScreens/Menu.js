...
const Menu = () => {
  ...
  const toggleAppliance = (category, appliance) => {
    if (!isConnected) {
      Alert.alert("Warning", "Cannot switch appliances without connection.");
      return;
    }
    setApplianceStatus((prevStatus) => ({
      ...prevStatus,
    ...
    setStatusToDevice(appliance);
  };

  const setStatusToDevice = async (device) => {
    try {
      await dmt3API.turnOffDevice(device);
    } catch (err) {
      console.log(err);
      Alert.alert("No running system", "Not connected to the system.");
    }
  ...
  const handleReport = async (appliance, status) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${baseURL}/api/ereport/send`, reportPayload);

      if (data.success) {
        Alert.alert("Report Submitted", data.message || "Report successfully sent.");
      } else {
        Alert.alert("Submission Failed", data.message || "Unknown error.");
      }
    } catch (error) {
      console.error("Error submitting report:", error);
      Alert.alert("Network Error", "Could not send the report.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const confirmReport = (appliance, status) => {
    Alert.alert(
      ...
      [
        ...
        { text: "OK", onPress: () => handleReport(appliance, status) },
      ],
      { cancelable: false }
    );
  };
                ...
                <View style={styles.cardbuttonContainer}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      confirmReport(
                        appliance.name,
                        applianceStatus[selected][appliance.name] ? "Active" : "Inactive"
                      )
                    }
                  >
